...
namespace JsonBox {
   ...
   //把string中的特殊字符，转换为json字符
    ...
    //value的[]的返回值，还是Value，参数是string

	Value &Value::operator[](const Object::key_type &key) {//这里的参数是string，因为object 的key是string
		if (type != OBJECT) {
			clear();
			type = OBJECT;
	...
	Value &Value::operator[](Array::size_type index) {
        ...
        if(index == (*data.arrayValue).size()) //索引超出了总长度，那么新建一个Value，追加到数组，并且返回他
        {
        ...
        {
            return (*(data.arrayValue))[index]; //返回数组索引处的Value
        }
	}
...
//                const char BEGIN_END_STRING = '"';
                  ...
                  if (input.good()) {
					if (currentCharacter == Structural::BEGIN_END_STRING) { // " 字符串
                    ...
                    else if (currentCharacter == Structural::BEGIN_OBJECT) { //{ 对象
						// The value to be parsed is an object.
						setObject(Object());
						readObject(input, *data.objectValue);
                        
                        std::cout<<"奶奶的，看看到底走没走"<<std::endl;
                        std::cout<<*data.objectValue<<std::endl;
                        
						noErrors = false;

					}
                    else if (currentCharacter == Structural::BEGIN_ARRAY) { //[ 数组
						...
						noErrors = false;
                             ...
                             (currentCharacter >= Numbers::DIGITS[0] && currentCharacter <= Numbers::DIGITS[9])) { //数字
	...
	void Value::readString(std::istream &input, std::string &result) {
		...
		while (noErrors && !input.eof()) {
			input.get(currentCharacter);
            std::cout<<"readstring="<<currentCharacter<<std::endl;
			...
			if (input.good()) {
				if (currentCharacter & 0x80) { // 0x80 --> 10000000
					...
					noErrors = false;
                    std::cout<<"字符串结束="<<result<<std::endl;
		...
		while (noErrors && !input.eof()) {
			...
			if (input.good()) {
				if (currentCharacter == Structural::BEGIN_END_STRING) {
					...
					currentCharacter = input.peek();//向前走一个，把最后一个分号过去
					...
					if (!input.eof()) {
						...
						if (currentCharacter == Structural::NAME_SEPARATOR) {
							...
							if (!input.eof()) {
								...
								// from the stream.
								input.putback(currentCharacter);
                                std::cout<<"之前的readObject="<<currentCharacter<<std::endl<<std::endl<<std::endl;
								...
								while (!input.eof() && currentCharacter != Structural::VALUE_SEPARATOR &&
                                    ...
                                    input.get(currentCharacter);
                                    std::cout<<"readObject="<<currentCharacter<<std::endl<<std::endl<<std::endl;
			...
			if (input.good()) {
				if (currentChar == Structural::END_ARRAY) {
					...
					while (!input.eof() && currentChar != ',' &&
						...
						input.get(currentChar);
                        std::cout<<"readArra="<<currentChar<<std::endl<<std::endl<<std::endl;
					}

					if (currentChar == Structural::END_ARRAY) {
						notDone = false;
                        std::cout<<"数组]走了吗"<<std::endl;
	...
	void Value::readNumber(std::istream &input, JsonBox::Value &result) {
		...
		while (notDone && !input.eof()) {
			...
			if (currentCharacter == '-') {
				...
				constructing << currentCharacter;
                std::cout<<"one by one="<<currentCharacter<<std::endl;
	...
	//参数二：表示输出来之后的字符串json是否压缩  参数三；啥转换
    ...
    //输出这个Value，因为Object和Array已经做了输出，所以简化了输出过程
